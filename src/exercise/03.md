...
React.memo can be used whenever unnecessary rerendering is happening and it causing performance issues. Also, if in any functional compoenent there are some calcuations based on props and are being assigned to the child compoenents then those calculations can be lift up in the parent compoenent so that React.memo can intelligently know when to rerender.
